  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    switch (sub) {
      ...
      case "reload" -> {
        ...
        send(
            sender,
            "  " + DOT + "&7Seeds Required: " + (cfg.requirePlayerSeed ? "&atrue" : "&cfalse"));
        send(sender, "");
        send(sender, LINE);
        return true;
      }
      case "status" -> {
        ...
        send(
            sender, "  " + (plugin.isCropEnabled(Material.CARROTS) ? "&a✔" : "&c✖") + " &7Carrots");
        send(
            sender,
            "  " + (plugin.isCropEnabled(Material.POTATOES) ? "&a✔" : "&c✖") + " &7Potatoes");
        send(
            sender,
            "  " + (plugin.isCropEnabled(Material.NETHER_WART) ? "&a✔" : "&c✖") + " &7Nether Wart");
        send(sender, "  " + (plugin.isCropEnabled(Material.COCOA) ? "&a✔" : "&c✖") + " &7Cocoa");
        send(
            sender,
            "  " + (plugin.isCropEnabled(Material.BEETROOTS) ? "&a✔" : "&c✖") + " &7Beetroots");
        send(
            sender,
            "  "
